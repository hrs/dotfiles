...
When writing a new message, start in insert mode (since I'm about to write
something).
...
Connect to SMTP servers over SSL.
...
Define the functions used to fetch mail, update the database, and change
accounts.
...
#+BEGIN_SRC emacs-lisp
  ...
  (defun hrs/reply-from-work-email ()
    (setq user-mail-address "harry@thoughtbot.com"
          smtpmail-smtp-server "smtp.gmail.com"))

  (defun hrs/refresh-notmuch-database ()
    (let ((mailboxes '("personal" "thoughtbot")))
      (shell-command-to-string "notmuch new")
      (dolist (mailbox mailboxes)
        (shell-command-to-string
         (format "notmuch tag +inbox \"path:%s/inbox/**\"" mailbox))
        (shell-command-to-string
         (format "notmuch tag +draft -inbox \"path:%s/drafts/**\"" mailbox))
        (shell-command-to-string
         (format "notmuch tag +sent -inbox \"path:%s/sent/**\"" mailbox))
        (shell-command-to-string
         (format "notmuch tag +archive -inbox \"path:%s/archive/**\"" mailbox)))))

  (defun hrs/fetch-new-mail ()
    (interactive)
    (save-excursion
      (shell-command-to-string "mbsync personal-inbox thoughtbot-inbox")
      (hrs/refresh-notmuch-database)))

  (defun hrs/sync-all-mailboxes ()
    (interactive)
    (shell-command-to-string "mbsync personal thoughtbot")
    (hrs/refresh-notmuch-database))
#+END_SRC

Bind keys to match what I'm used to from =mutt=.

I might eventually change these back to the defaults, but right now I'm just
trying to make the transition easy for myself.
...
Open links in the usual org-ly way with =C-c C-o=.

#+BEGIN_SRC emacs-lisp
  (define-key notmuch-show-mode-map (kbd "C-c C-o") 'org-open-at-point)
#+END_SRC

Default to using my personal address.

#+BEGIN_SRC emacs-lisp
  (hrs/reply-from-personal-email)
#+END_SRC
