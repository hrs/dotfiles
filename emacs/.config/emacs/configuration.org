...
#+begin_src emacs-lisp
  (use-package minions
    ...
    :config
    (defun hrs-set-minions-mode-line-lighter (_)
      (setq minions-mode-line-lighter
            (if (display-graphic-p) "⚙" "#")))

    (add-to-list 'after-make-frame-functions #'hrs-set-minions-mode-line-lighter)
    (minions-mode 1))
#+end_src
...
=~/documents/org/archive/archive-YYYY-MM.org=.
...
#+begin_src emacs-lisp
  ...
  (setq org-archive-location
        (concat
         (org-file-path (format-time-string "archive/archive-%Y-%m.org"))
         "::* From %s"))
...
#+begin_src emacs-lisp
  (use-package denote
    ...
    :bind (("C-c n b" . denote-link-backlinks)
           ("C-c n f" . hrs-denote-find-file)
           ("C-c n l" . denote-link)
           ("C-c n n" . denote)
           ("C-c n s" . hrs-denote-search))

    :custom
    (denote-directory "~/documents/notes")

    :config
    (defun hrs-denote-find-file ()
      (interactive)
      (projectile-find-file-in-directory denote-directory))

    (defun hrs-denote-search (term)
