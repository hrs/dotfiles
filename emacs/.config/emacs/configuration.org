...
Update all my feeds every five minutes, assuming Emacs has been idle for at
least 30 seconds.

#+begin_src emacs-lisp
  (run-with-idle-timer 30 300 'elfeed-update)
#+end_src
...
I record minutes from client meetings with denote. I make that a little more
convenient by using completion to automatically associate some appropriate
keywords with the note and prompting for a list of attendees.

#+begin_src emacs-lisp
  (cl-defstruct project name slug people)

  (defvar +active-projects '()
    "I don't publish my active project list, so this is redefined
    elsewhere.")

  (defun +project-with-name (name)
    (seq-find (lambda (project) (string= name (project-name project)))
              +active-projects))

  (defun +choose-project ()
    (+project-with-name (completing-read "Project: "
                                         (mapcar #'project-name +active-projects))))

  (defun +meeting-template (project)
    (let ((attendees (cons "Harry Schwartz"
                           (completing-read-multiple "Attendees: "
                                                     (project-people project)))))
      (string-join `("* Attendees" ""
                     ,@(mapcar (lambda (attendee) (format "- %s" attendee)) attendees) ""
                     "* Notes" ""
                     "* Next Steps" "") "\n")))

  (defun +denote-log-meeting ()
    "Create a `denote' entry logging a meeting."
    (interactive)
    (let* ((title (denote-title-prompt "Planning"))
           (project (+choose-project))
           (default-keywords '("meeting" "work"))
           (keywords (if project
                         (cons (project-slug project) default-keywords)
                       default-keywords))
           (template (+meeting-template project)))
