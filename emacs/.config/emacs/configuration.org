...
I've got a single big =bibtex= file that contains all the references in my
notes. That's mostly books, but also some papers and Web sites.
...
Set up =org-similarity= to score my notes.

#+begin_src emacs-lisp
  (use-package org-similarity
    :load-path "~/code/org-similarity"

    :custom
    (org-similarity-directory denote-directory)
    (org-similarity-show-scores 10)
    (org-similarity-recursive-search t))
#+end_src

Calling =+denote-similar-unlinked-notes-sidebuffer= opens a buffer of links to
nodes that are (1) textually similar to this note, and (2) not yet linked from
it.

Just an experiment so far. I'll probably break this into a mode or something
eventually.
...
#+begin_src emacs-lisp
  ...
  (defun +denote-ids-in-file (file-name)
    ""
    (+denote-ids-in-string (with-temp-buffer
                             (insert-file-contents file-name)
                             (buffer-string))))

  (defun +denote-similar-unlinked-notes-org (file-name)
    ""
    (let ((raw-similarity-results (+org-similarity-results file-name))
          (linked-note-ids (+denote-ids-in-file file-name)))
      (s-join "\n"
              (-filter (lambda (line)
                         (not (-some (lambda (string-to-remove)
                                       (s-contains? string-to-remove line))
                                     linked-note-ids)))
                       (s-lines raw-similarity-results)))))

  (defun +denote-similar-unlinked-notes-sidebuffer (file-name)
    ...
    (if file-name
        (let ((sidebar-buffer (get-buffer-create (format "*similar notes: %s*"
          ...
          (with-current-buffer sidebar-buffer
            (erase-buffer)
            (insert (+denote-similar-unlinked-notes-org file-name))
            (org-mode))

          (display-buffer-in-side-window sidebar-buffer
                                         '((side . right) (slot . -1)))
          (select-window (get-buffer-window sidebar-buffer)))

      (error "can't compare this buffer (have you saved it?)")))

  (defun +org-similarity-results (file-name)
    (shell-command-to-string
     (format "%s -m orgsimilarity -i %s -d %s -l %s -n %s %s %s %s"
