...
#+begin_src emacs-lisp
  (defun +dashboard ()
    ...
    (call-process-shell-command "copy-phone-notes")
...
Automatically save the database automatically, since =elfeed= doesn't. This adds
some resilience if Emacs crashes and helps synchronize state across machines.
