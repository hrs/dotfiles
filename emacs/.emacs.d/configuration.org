...
work-related tasks, so I'd like to derive my agenda from those two files.
...
The "Personal agenda" view is a bit complicated; it searches only my personal
list of tasks (that is, not including work tasks) and displays:

1. The high-priority tasks (that is, the ones marked =[#A]=),
2. My agenda for the next few days,
...
Those are structured as =org-habit= items so they can recur according to a
schedule.
...
=skip-subtree-if-priority= function, which I've shamelessly stolen and
re-prefixed.
...
#+begin_src emacs-lisp
  ...
  (setq org-agenda-custom-commands
        '(("p" "Personal agenda"
          ...
          ("w" "Workday agenda"
           ((agenda "") (todo "TODO") (todo "PENDING"))
           ((org-agenda-files (list (org-file-path "work.org")))))))
#+end_src

I consult my agenda pretty often, so I bind =C-c d= to open up a full-screen
version of it.
