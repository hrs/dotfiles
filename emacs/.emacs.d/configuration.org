...
When I'm composing a new email, default to using the current context (e.g., if
I'm in my personal inbox, send mail from my personal context). This is almost
...
Compose new emails (as with =C-x m=) using =mu4e-user-agent=.
